...
To allow an Enterprise administrator to centrally manage backups, child accounts must authorize the enterprise's central backup service to interact with their backup snapshot resources. Specifically, the central backup service needs to be authorized to work with **Block Storage for VPC**, **Snapshots for VPC**, and **Virtual Server for VPC services**. Authorization templates can simplify the creation and management of these relationships. 
